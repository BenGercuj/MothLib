...
void Window::StartGame()
{
    ...
    for (int i = 0; i < XX/20; i++)
    {
        for (int j = 0; j < YY/20; j++)
        {
            ...
            std::function f = [this](int posx, int posy, int &whowasit, std::string &text)
            {
                if (!_field[posx][posy])
                {
                    _field[posx][posy] = _playerid;
                    whowasit = _playerid;
                    if (_playerid == 1)
                    {
                        text = "X";
            ...
            if (i == XX/20-1 && j != YY/20-1) { btn = new Button(this, (i*XX/20), (j*YY/20)+YY/20+2, XX/20-4, YY/20, name, "", f, i, j); }
            else if (j == YY/20-1 && i != XX/20-1) { btn = new Button(this, (i*XX/20), (j*YY/20)+YY/20+2, XX/20, YY/20-1, name, "", f, i, j); }
            else if (j == YY/20-1 && i == XX/20-1) { btn = new Button(this, (i*XX/20), (j*YY/20)+YY/20+2, XX/20-4, YY/20-4, name, "", f, i, j); }
            else { btn = new Button(this, (i*XX/20), (j*YY/20)+YY/20+2, XX/20, YY/20, name, "", f, i, j); }
        }
    }
}
